...
public class A2_MatchPhoneNumber {
    public static void main(String[] args) {
        ...
        List<String> phoneMatchesList = new ArrayList<>();

        while (matcher.find()) {
            phoneMatchesList.add(matcher.group());
        }

        System.out.println(String.join(", ", phoneMatchesList));
    }
}
...
· Use a capturing group to ensure the delimiter is only one of the allowed characters (space or hyphen) and not a
combination of both (e.g., +359 2-111 111 has mixed delimiters, it is invalid). Use a group back reference to achieve this.
